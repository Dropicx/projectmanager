...
async function testUsageLimiter() {
  ...
  try {
    ...
    if (novaLiteCheck.stats) {
      console.log(
        `   Monthly: $${(novaLiteCheck.stats.monthlyUsed / 100).toFixed(2)} / $${(novaLiteCheck.stats.monthlyLimit / 100).toFixed(2)}`
      );
      console.log(
        `   Daily: $${(novaLiteCheck.stats.dailyUsed / 100).toFixed(2)} / $${(novaLiteCheck.stats.dailyLimit / 100).toFixed(2)}`
      );
    }
    ...
    console.log(
      "\nClaude 3.7 Sonnet (10k tokens):",
      claudeCheck.allowed ? "✅ Allowed" : "❌ Blocked"
    );
    if (claudeCheck.stats) {
      console.log(
        `   Monthly: $${(claudeCheck.stats.monthlyUsed / 100).toFixed(2)} / $${(claudeCheck.stats.monthlyLimit / 100).toFixed(2)}`
      );
      console.log(
        `   Daily: $${(claudeCheck.stats.dailyUsed / 100).toFixed(2)} / $${(claudeCheck.stats.dailyLimit / 100).toFixed(2)}`
      );
    }
    ...
    console.log(
      `Monthly usage: $${(stats.monthlyUsed / 100).toFixed(2)} / $${(stats.monthlyLimit / 100).toFixed(2)} (${stats.percentUsed.toFixed(1)}%)`
    );
    console.log(
      `Daily usage: $${(stats.dailyUsed / 100).toFixed(2)} / $${(stats.dailyLimit / 100).toFixed(2)}`
    );
