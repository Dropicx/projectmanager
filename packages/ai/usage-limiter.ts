...
export class UsageLimiter {
  ...
  private async resetDailyUsageIfNeeded(organizationId: string, org: any): Promise<void> {
    ...
    if (!lastUpdate || now.getDate() !== lastUpdate.getDate() || now.getMonth() !== lastUpdate.getMonth()) {
      await db
        .update(organizations)
        .set({
  ...
  private async checkAndSendAlerts(organizationId: string): Promise<void> {
    ...
    if (stats.percentUsed >= 90) {
      console.warn(`⚠️ Organization ${organizationId} at ${stats.percentUsed.toFixed(1)}% of monthly budget`)
    ...
    if (stats.dailyUsed >= stats.dailyLimit * 0.9) {
      console.warn(`⚠️ Organization ${organizationId} at ${((stats.dailyUsed / stats.dailyLimit) * 100).toFixed(1)}% of daily limit`)
  ...
  getDefaultLimitsForTier(tier: 'free' | 'pro' | 'enterprise'): { monthly: number; daily: number } {
    ...
    const defaultMonthly = parseFloat(process.env.BEDROCK_MONTHLY_BUDGET_USD || '100') * 100
